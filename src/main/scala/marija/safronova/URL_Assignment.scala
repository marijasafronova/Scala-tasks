...
class Document (val title: String = "",
  ...
  val wordCount: Int = URLreading.getWordCountPerLine(rows).sum
  ...
  def Save(dst: String = "", folder: String = "src/resources/texts"): Unit = {
    ...
    if (dst.isEmpty) {
      ...
      URLreading.saveText(folder+'/'+filename, stringBuilder.mkString)
    }
    else {
      URLreading.saveText(folder+'/'+dst, stringBuilder.mkString)
    }
  }

}
object URL_Assignment extends App {
